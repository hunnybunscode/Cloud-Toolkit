...
if [[ "$missing" -eq 1 ]]; then
  ...
  bash "$SCRIPT_DIR/setup_dependencies.sh" || warn "setup_dependencies.sh returned non-zero; continuing"
else
...
run_action() {
  ...
  if [[ ! -f "$SCRIPT_DIR/$script" ]]; then fail "Action script not found: $script"; fi
...
while true; do
  ...
  case "${choice:-}" in
    1) run_action "action_encrypt_ebs.sh"         "action_encrypt_ebs" ;;
    2) run_action "connect_ec2_ssm.sh"            "connect_ec2_ssm"    ;;
    3) run_action "action_enable_ebs_default.sh"  "action_enable_ebs_default" ;;
    4) run_action "action_audit_s3_public.sh"     "action_audit_s3_public" ;;
    5) run_action "action_audit_open_sg_rules.sh" "action_audit_open_sg_rules" ;;
    6) run_action "action_test.sh"                "action_test" ;;
    0) echo "Bye."; exit 0 ;;
